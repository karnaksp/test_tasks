...
def filter_train_data(
    train: pd.DataFrame, test: pd.DataFrame, k: int = 25
) -> Tuple[pd.DataFrame, pd.DataFrame, float]:
    """
    Фильтрует необработанные данные на основе частоты появления пользователей в подмножестве.

    Args:
        train (pd.DataFrame): train DataFrame содержит историю покупок пользователей.
        test (pd.DataFrame): testmission DataFrame содержит последовательность сочетаний юзер-категория, которые нужно предсказать.
    ...
    """
    users = map(int, (x.split(";")[0] for x in test["id"]))
    user_counts = Counter(users)
    frequent_users = {user for user, count in user_counts.items() if count > k}
    test_users = {user for user, count in user_counts.items()}
    filtered_train = train[train["user_id"].isin(frequent_users)]
    filtered_test = train[train["user_id"].isin(test_users)]
    ...
    return filtered_train, filtered_test, proportion


def ohe_data(train: pd.DataFrame) -> pl.DataFrame:
    """
    ...
    train_train = pd.get_dummies(train, columns=["cart"])
    train_train = pl.from_pandas(train_train)
    train_train = train_train.group_by(["user_id", "order_completed_at"]).max()
    train_train = train_train.sort(["user_id", "order_completed_at"])

    return train_train


def full_history(train_train: pl.DataFrame) -> pl.DataFrame:
    """
    Обрабатывает датафрейм polars и возвращает весь датасет.

    Args:
        train (pl.DataFrame): Polars DataFrame для обработки.
    ...
    train_train = train_train.with_columns(
    ...
    train_train = train_train.drop("order_completed_at")
...
class MarketDataProcessor:
    ...
    #         "hour", "week", "weekday", "day", "month", "year"
    #     )
    #     # Преобразование в длинный формат пользователя и категории товара
    #     if not history_flag:
    #         train_melt = train_tmp.drop("order_number").melt(
    #             id_vars=["user_id", "hour", "week", "weekday", "day", "month", "year"],
    #             variable_name="category",
    ...
    #         exit()
    #     # Тренировочный датасет
    #     train_set = train_melt

    #     order_number_df = val_data.select(
    #         ["user_id", "order_number"]
    #     ).unique()  # Количество заказов у пользователя
    #     dl_tmp = train_set.group_by("category").agg(
    #         pl.col(["ordered"]).sum()
    #     )  # Количество заказов по категории
    #     if history_flag == 1:
    #         order_number_df = order_number_df.with_columns(pl.col("order_number") + 1)
    ...
    #     )
    #     # Вычисление рейтинга по времени
    #     rating_per_hour = train_set.group_by(
    #         ["year_mean", "month_mean", "hour_mean", "user_id"]
    #     ).agg(
    #         ((pl.col("total_rating_mean").mean()) * 100).alias("rating_per_hour_mean"),
    #         ((pl.col("total_rating_mean").std()) * 100).alias("rating_per_hour_std"),
    ...
    #             "rating_per_hour_median"
    #         ),
    #     )

    #     rating_per_weekday = train_set.group_by(
    #         ["year_mean", "month_mean", "weekday_mean", "user_id"]
    #     ).agg(
    #         ((pl.col("total_rating_mean").mean()) * 100).alias("rating_per_w_mean"),
    #         ((pl.col("total_rating_mean").std()) * 100).alias("rating_per_w_std"),
    ...
    #         ((pl.col("total_rating_mean").median()) * 100).alias("rating_per_w_median"),
    #     )
    #     rating_per_day = train_set.group_by(
    #         ["year_mean", "month_mean", "day_mean", "user_id"]
    #     ).agg(
    #         ((pl.col("total_rating_mean").mean()) * 100).alias("rating_per_d_mean"),
    #         ((pl.col("total_rating_mean").std()) * 100).alias("rating_per_d_std"),
    ...
    #     rating_per_month = train_set.group_by(
    #         ["year_mean", "month_mean", "user_id"]
    #     ).agg(
    #         ((pl.col("total_rating_mean").mean()) * 100).alias("rating_per_m_mean"),
    #         ((pl.col("total_rating_mean").std()) * 100).alias("rating_per_m_std"),
    #         ((pl.col("total_rating_mean").sum())).alias("rating_per_m_sum"),
    ...
    #         rating_per_month, on=["year_mean", "month_mean", "user_id"]
    #     )
    #     train_set = train_set.join(
    #         rating_per_weekday,
    #         on=["year_mean", "month_mean", "weekday_mean", "user_id"],
    #     )
    #     train_set = train_set.join(
    #         rating_per_day, on=["year_mean", "month_mean", "day_mean", "user_id"]
    #     )
    ...
    #     if not history_flag:
    #         # Присоединение целевой переменной из valid_melt к train_set
    #         valid_melt = val_data.drop(
    #             "order_number",
    #         ).melt(
    #             id_vars=[
    ...
    #     #     train_set = train_set.drop('user_id', 'category')
    ...
    def _calculate_rating_per_time(self, train_set, time_column):
        ...
        rating_per_time = train_set.group_by([f"{time_column}_mean", "user_id"]).agg(
            ...
            ((pl.col("total_rating_mean").sum())).alias(
                f"rating_per_{time_column}_sum"
            ),
            ((pl.col("total_rating_mean").median()) * 100).alias(
