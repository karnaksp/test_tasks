...
def goods_on_start(hashMap, _files=None, _data=None):
    ...
    query = select(c for c in SW_Goods)
        ...
        list["customcards"]["cardsdata"].append(
            {
                "name": record.name,
                "key": record.id,
                "product_number": str(record.product_number),
                "barcode": str(record.barcode),
                "unit": str(record.unit),
                "skugroup": str(record.group),
                "price": record.price,
...
def goods_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "btn_add":
        ...
        nom = SW_Goods.get(barcode=hashMap.get("barcode_search"))
        if nom == None:
        ...
        nom = SW_Goods.get(id=int(hashMap.get("nom_id")))
        if nom == None:
            hashMap.put("toast", "Товар не найден")
        else:
            hashMap, nom_id = open_nom(hashMap, nom_id, str(nom.id))
...
def goods_record_on_start(hashMap, _files=None, _data=None):
    ...
    hashMap.put(
        "fill_barcode",
        json.dumps({"hint": "Штрихкод", "default_text": hashMap.get("barcode")}),
    )
    hashMap.put(
        "fill_product_number",
        json.dumps({"hint": "Артикул", "default_text": hashMap.get("product_number")}),
    )
    hashMap.put(
        "fill_price", json.dumps({"hint": "Цена", "default_text": hashMap.get("price")})
    )
    ...
    if nom_id < 0:
        ...
        with db_session:
            r = SW_Goods(
                ...
                barcode=get_if_exist(hashMap, "barcode"),
                unit=get_if_exist(hashMap, "unit"),
                group=get_if_exist(hashMap, "group"),
...
def goods_record_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "btn_save":
        ...
        )  # "переменная"+"_path" - сюда помещается путь к полученной фотографии
        ...
        hashMap.get("listener") == "gallery_change"
    ):  # пользователь может удалить фото из галереи. Новый массив надо поместить к документу

        if hashMap.containsKey(
            "photoGallery"
        ):  # эти 2 обработчика - аналогичные, просто для разных событий
            jphotoarr = json.loads(hashMap.get("photoGallery"))
            hashMap.put("photoGallery", json.dumps(jphotoarr))
...
def cells_on_start(hashMap, _files=None, _data=None):
    ...
    query = select(c for c in SW_Cells)
    list["customcards"]["cardsdata"] = []

    for record in query:

        list["customcards"]["cardsdata"].append(
            {"name": record.name, "key": record.id, "barcode": str(record.barcode)}
        )
    hashMap.put("list", json.dumps(list))
...
def cells_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "btn_add":
        ...
        jrecord = next(
            ...
            if str(item["key"]) == hashMap.get("selected_card_key")
        )
...
def income_cell_on_start(hashMap, _files=None, _data=None):
    ...
    jt = {
        ...
        "hideinterline": "false",
        "columns": [
            {"name": "cell", "header": "Ячейка", "weight": "1"},
            {"name": "nom", "header": "Товар", "weight": "2"},
            {"name": "qty", "header": "Кол-во", "weight": "1"},
        ],
        "rows": [],
    }
...
def income_cell_on_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "":
        ...
        cell = SW_Cells.get(barcode=hashMap.get("barcode_cell"))
        if cell == None:
        ...
        cell = SW_Cells.get(name=hashMap.get("cell"))
        if cell == None:
            hashMap.put("toast", "Ячейка не найдена")
        else:
            hashMap.put("cell", cell.name)
...
def income_nom_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "":
        ...
        nom = SW_Goods.get(barcode=hashMap.get("barcode"))
        if nom == None:
            hashMap.put("toast", "Товар не найден")
        else:
...
def income_qty_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "":
        ...
        if getfloat_if_exist(hashMap, "qty") > 0:
            with db_session:
                r = SW_Account(
                    qty=getfloat_if_exist(hashMap, "qty"),
                    sku=int(hashMap.get("nom_id")),
...
def outcome_qty_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "":
        ...
        if getfloat_if_exist(hashMap, "qty") > 0:
            with db_session:
                r = SW_Account(
                    qty=getfloat_if_exist(-1 * hashMap, "qty"),
                    sku=int(hashMap.get("nom_id")),
                    cell=int(hashMap.get("cell_id")),
                )
                commit()
...
def stock_on_create(hashMap, _files=None, _data=None):
    ...
    table = {
        ...
        "columns": [
            {"name": "cell", "header": "Ячейка", "weight": "1"},
            {"name": "nom", "header": "Товар", "weight": "2"},
            {"name": "qty", "header": "Остаток", "weight": "1"},
        ],
    }
    ...
    try:
        conn = sqlite3.connect(
            "//data/data/ru.travelfood.simple_ui/databases/SimpleWMS"
        )
    except Error as e:
...
def get_table_by_cell(id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT SW_Cells.name as cell,SW_Goods.name as nom, ifnull(sum(qty),0) as qty FROM SW_Account LEFT JOIN SW_Goods ON SW_Account.sku=SW_Goods.id LEFT JOIN SW_Cells ON SW_Account.cell=SW_Cells.id WHERE SW_Account.cell = "
        + str(id)
        + " GROUP BY SW_Goods.name ,SW_Cells.name HAVING ifnull(sum(qty),0)<>0"
    )
...
def get_table_by_nom(id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT SW_Cells.name as cell,SW_Goods.name as nom, ifnull(sum(qty),0) as qty FROM SW_Account LEFT JOIN SW_Goods ON SW_Account.sku=SW_Goods.id LEFT JOIN SW_Cells ON SW_Account.cell=SW_Cells.id WHERE SW_Account.sku = "
        + str(id)
        + " GROUP BY SW_Goods.name ,SW_Cells.name HAVING ifnull(sum(qty),0)<>0"
    )
...
def stock_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "barcode":
        ...
        if not is_nom:
            cell = SW_Cells.get(barcode=hashMap.get("barcode"))
            if cell == None:
...
def inventory_on_start(hashMap, _files=None, _data=None):
    ...
    query = select(c for c in SW_Inventory)
    list["customcards"]["cardsdata"] = []
...
def newinv_input(hashMap, _files=None, _data=None):
    ...
    if hashMap.get("listener") == "":
        with db_session:
            r = SW_Inventory(description=hashMap.get("name"))
            commit()
...
def invres_on_start(hashMap, _files=None, _data=None):
    ...
    hashMap.put(
        "inv",
        "Инвентаризация "
        + str(hashMap.get("inv_name"))
        + " от "
        + invdate.strftime("%m.%d.%Y, %H:%M:%S"),
    )
    ...
    table = {
        ...
        "columns": [
            {"name": "cell", "header": "Ячейка", "weight": "1"},
            {"name": "nom", "header": "Товар", "weight": "2"},
            {"name": "qtyplan", "header": "План", "weight": "1"},
            {"name": "qtyfact", "header": "Факт", "weight": "1"},
        ],
    }
...
def invcv_cell_on_new_object_ocr(hashMap, _files=None, _data=None):
    ...
    if not cellid == None:
        ...
        try:
            ...
            cursor.execute(
                'SELECT SW_Goods.product_number as barcode,SW_Cells.name as cell,SW_Goods.name as nom, ifnull(sum(qty),0) as qty, "unique" as un,SW_Goods.id as nom_id  FROM SW_Account LEFT JOIN SW_Goods ON SW_Account.sku=SW_Goods.id LEFT JOIN SW_Cells ON SW_Account.cell=SW_Cells.id WHERE SW_Account.cell = '
                + str(cellid)
                + " GROUP BY SW_Goods.name ,SW_Cells.name HAVING ifnull(sum(qty),0)<>0"
            )
        except Error as e:
        ...
        for link in results:
            job = {
                "object": str(link[0]),
                "info": str(link[2]) + " </n> Остаток: <big>" + str(link[3]) + "</big>",
                "unique": link[4],
                "nom_id": str(link[5]),
            }
            info_list.append(job)
...
def invcv_cell_on_new_object(hashMap, _files=None, _data=None):
    ...
    if not cellid == None:
        ...
        try:
            # тут я понял что лажанулся, использовав unique в качестве имени, но было уже поздно
            cursor.execute(
                'SELECT SW_Goods.barcode as barcode,SW_Cells.name as cell,SW_Goods.name as nom, ifnull(sum(qty),0) as qty, "unique" as un,SW_Goods.id as nom_id  FROM SW_Account LEFT JOIN SW_Goods ON SW_Account.sku=SW_Goods.id LEFT JOIN SW_Cells ON SW_Account.cell=SW_Cells.id WHERE SW_Account.cell = '
                + str(cellid)
                + " GROUP BY SW_Goods.name ,SW_Cells.name HAVING ifnull(sum(qty),0)<>0"
            )
        except Error as e:
...
def invcv_goods_on_new_object(hashMap, _files=None, _data=None):
    ...
    try:
        nom_record = next(
            item for item in object_list if str(item["object"]) == str(nom_barcode)
        )
    ...
    if not nom_record == None:
        ...
        if nom_record["unique"] == 1:
            ...
            with db_session:
                ...
                inventory = SW_Inventory[int(hashMap.get("inv_id"))]
                r = SW_Inventory_line(
                    qty=1,
                    sku=int(hashMap.get("nom_id")),
                    cell=cellid,
